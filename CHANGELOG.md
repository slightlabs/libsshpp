...
* CMake build (compiled + install rules), Conan-ready structure, Catch2 unit
  tests, and integration tests that spin up a real ephemeral `sshd` (or, for
  the server module, use `libsshpp`'s own client and server together).
...
`SimpleAuthHandler`/`SftpSubsystemHandler`; header-only mode
(`LIBSSHPP_HEADER_ONLY=ON`) is scaffolded in CMake but not yet functional.
