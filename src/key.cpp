...
Key Key::from_native(native_key n, Ownership o) { return Key(n, o == Ownership::owning); }

Result<Key> Key::from_private_file(const std::filesystem::path& path, PassphraseCallback cb) {
    ssh_key key = nullptr;
...
Result<Key> Key::from_public_file(const std::filesystem::path& path) {
    ssh_key key = nullptr;
...
Result<Key> Key::from_public_base64(std::string_view b64, KeyType type) {
    ssh_key key = nullptr;
...
Result<Key> Key::from_authorized_keys_line(std::string_view line) {
...
Result<std::string> Key::to_public_base64() const {
...
Result<std::string> Key::to_authorized_keys_line(std::string_view comment) const {
...
Result<void> Key::write_public_file(const std::filesystem::path& path) const {
...
Result<Key> Key::public_part() const {
...
bool Key::is_private() const noexcept { return native_ != nullptr && ssh_key_is_private(native_) != 0; }
bool Key::is_public() const noexcept { return native_ != nullptr && ssh_key_is_public(native_) != 0; }

Result<Fingerprint> Key::fingerprint(HashType type) const {
...
bool Key::equals(const Key& other, bool compare_private) const noexcept {
...
Result<Key> Key::generate(KeyType type, int bits) {
    ssh_key key = nullptr;
...
bool Fingerprint::operator==(const Fingerprint& other) const noexcept {
