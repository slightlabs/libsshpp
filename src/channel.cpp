...
Channel Channel::from_native(native_channel n, detail::SessionCorePtr core, Ownership o) {
    Channel ch(n, std::move(core), o == Ownership::owning);
    ch.open_ = n != nullptr && ssh_channel_is_open(n) != 0;
    return ch;
}

Result<void> Channel::try_open_session() {
...
Result<void> Channel::try_request_pty(std::string_view term, PtySize size) {
...
Result<void> Channel::try_change_pty_size(PtySize size) {
...
Result<void> Channel::try_request_shell() {
...
Result<void> Channel::try_request_exec(std::string_view command) {
...
Result<void> Channel::try_request_subsystem(std::string_view name) {
...
Result<void> Channel::try_request_env(std::string_view name, std::string_view value) {
...
Result<void> Channel::try_send_signal(Signal s) {
...
Result<std::size_t> Channel::try_read_some(MutableByteView buf, Stream stream) {
...
Result<std::string> Channel::try_read_all(Stream stream, std::size_t limit) {
    std::string result;
...
Result<std::size_t> Channel::try_write_some(ByteView data) {
...
Result<void> Channel::try_write_all(ByteView data) {
...
Result<void> Channel::try_write_all(std::string_view data) {
    return try_write_all(ByteView(data));
}

Result<std::size_t> Channel::try_bytes_available(Stream stream) const {
...
Result<bool> Channel::try_wait_readable(std::chrono::milliseconds timeout, Stream stream) {
...
Result<void> Channel::try_send_eof() {
...
bool Channel::is_eof() const noexcept { return native_ != nullptr && ssh_channel_is_eof(native_) != 0; }

Result<void> Channel::try_close() {
...
Result<ExitState> Channel::try_wait_exit(std::chrono::milliseconds timeout) {
...
ExitState Channel::exit_state() const noexcept {
    ExitState st;
