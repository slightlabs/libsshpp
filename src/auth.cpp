...
AuthMethods AuthMethods::from_bits(int bits) noexcept {
    AuthMethods m;
...
int AuthMethods::to_bits() const noexcept {
    int bits = 0;
...
Result<AuthStatus> None::attempt(detail::SessionCore& core) const noexcept {
    int rc = ssh_userauth_none(core.raw(), nullptr);
    return map_auth_result(rc, core.raw(), "ssh_userauth_none");
}

Result<AuthStatus> Password::attempt(detail::SessionCore& core) const noexcept {
    int rc = ssh_userauth_password(core.raw(), nullptr, password_.c_str());
    return map_auth_result(rc, core.raw(), "ssh_userauth_password");
}

Result<AuthStatus> PublicKeyAuto::attempt(detail::SessionCore& core) const noexcept {
...
Result<PublicKey> PublicKey::from_file(const std::filesystem::path& p, PassphraseCallback cb) {
    auto key = Key::from_private_file(p, std::move(cb));
    if (!key) return key.error();
    return auth::PublicKey(std::move(*key));
}

Result<AuthStatus> PublicKey::attempt(detail::SessionCore& core) const noexcept {
...
Result<AuthStatus> Agent::attempt(detail::SessionCore& core) const noexcept {
    int rc = ssh_userauth_agent(core.raw(), nullptr);
    return map_auth_result(rc, core.raw(), "ssh_userauth_agent");
}

KeyboardInteractive KeyboardInteractive::with_password(SecureString password) {
...
Result<AuthStatus> KeyboardInteractive::attempt(detail::SessionCore& core) const noexcept {
...
Result<AuthStatus> Chain::attempt(detail::SessionCore& core) const noexcept {
