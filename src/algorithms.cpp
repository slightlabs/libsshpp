...
Result<std::uint64_t> try_download(Sftp& sftp, const RemotePath& remote,
...
Result<std::uint64_t> try_upload(Sftp& sftp, const std::filesystem::path& local,
...
Result<std::string> try_read_file(Sftp& sftp, const RemotePath& path, std::size_t limit) {
...
Result<void> try_write_file(Sftp& sftp, const RemotePath& path, ByteView data,
...
Result<TreeStats> try_download_tree(Sftp& sftp, const RemotePath& remote,
                                    const std::filesystem::path& local, TransferOptions opts) {
    TreeStats stats;
    std::error_code ec;
