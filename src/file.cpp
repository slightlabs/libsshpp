...
File::ReadAhead::ReadAhead(File& file, std::size_t chunk, std::size_t depth)
...
#if SSHPP_HAS_SFTP_AIO
Result<void> File::ReadAhead::submit_one() {
...
Result<ByteView> File::ReadAhead::try_next() {
...
File::WriteBehind::WriteBehind(File& file, std::size_t chunk, std::size_t depth)
    : file_(&file), chunk_(chunk != 0 ? chunk : kDefaultChunk), depth_(depth != 0 ? depth : kDefaultDepth) {}

File::WriteBehind::~WriteBehind() {
    auto r = try_flush();
    if (!r) Library::report_destructor_error(r.error());
}

#if SSHPP_HAS_SFTP_AIO
Result<void> File::WriteBehind::drain_one() {
    native_sftp_aio aio = inflight_.front();
    inflight_.pop_front();
...
Result<void> File::WriteBehind::try_write(ByteView data) {
...
Result<void> File::WriteBehind::try_flush() {
#if SSHPP_HAS_SFTP_AIO
