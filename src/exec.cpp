...
const ExecResult& ExecResult::check() const {
    if (exit_code != 0) {
        ErrorInfo info;
...
Exec& Exec::env(std::string_view name, std::string_view value) {
    env_.emplace_back(std::string(name), std::string(value));
    return *this;
}

Exec& Exec::pty(bool enable, std::string_view term, PtySize size) {
    pty_ = enable;
...
Exec& Exec::stdin_data(std::string data) {
    stdin_data_ = std::move(data);
    return *this;
}

Result<ExecResult> Exec::try_run(std::string_view command) {
...
ExecResult Exec::run(std::string_view command) { return try_run(command).value(); }

Result<ExecResult> Exec::try_run(const std::vector<std::string>& argv) {
    std::string command;
