...
Attributes internal::attributes_from_native(sftp_attributes raw) {
    Attributes a;
...
Sftp::Sftp(Session& session) : native_(sftp_new(session.native_handle())), core_(session.core_) {}
...
int Sftp::protocol_version() const noexcept {
    return native_ != nullptr ? sftp_server_version(native_) : 0;
}

Result<Limits> Sftp::try_limits() const {
...
Result<File> Sftp::try_open(const RemotePath& path, OpenMode mode, std::filesystem::perms create_perms) {
...
Result<Attributes> Sftp::try_stat(const RemotePath& path) const {
...
Result<Attributes> Sftp::try_lstat(const RemotePath& path) const {
...
Result<bool> Sftp::try_exists(const RemotePath& path) const {
...
Result<void> Sftp::try_chmod(const RemotePath& path, std::filesystem::perms perms) {
...
Result<void> Sftp::try_chown(const RemotePath& path, std::uint32_t uid, std::uint32_t gid) {
...
Result<void> Sftp::try_mkdir(const RemotePath& path, std::filesystem::perms perms) {
...
Result<void> Sftp::try_mkdir_p(const RemotePath& path, std::filesystem::perms perms) {
...
Result<void> Sftp::try_rmdir(const RemotePath& path) {
...
Result<void> Sftp::try_remove(const RemotePath& path) {
...
Result<void> Sftp::try_rename(const RemotePath& from, const RemotePath& to) {
...
Result<void> Sftp::try_symlink(const RemotePath& target, const RemotePath& link) {
...
Result<RemotePath> Sftp::try_readlink(const RemotePath& path) const {
...
Result<RemotePath> Sftp::try_canonicalize(const RemotePath& path) const {
...
Result<Directory> Sftp::try_open_directory(const RemotePath& path) const {
...
Result<std::vector<Attributes>> Sftp::try_list(const RemotePath& path) const {
...
Result<DirectoryRange> entries(Sftp& sftp, const RemotePath& path) {
