...
// Generated from src/host_key_verifier.cpp; see docs/design/09 §9.3.
...
SSHPP_INLINE HostKeyVerifier::Decision StrictHostKeyPolicy::verify(const Context& ctx) const {
    return ctx.status == KnownHostsStatus::ok ? Decision::accept : Decision::reject;
}

SSHPP_INLINE HostKeyVerifier::Decision TofuHostKeyPolicy::verify(const Context& ctx) const {
    switch (ctx.status) {
...
SSHPP_INLINE HostKeyVerifier::Decision AcceptAnyHostKeyPolicy::verify(const Context&) const {
    return Decision::accept;
}

SSHPP_INLINE HostKeyVerifier::Decision PinnedHostKeyPolicy::verify(const Context& ctx) const {
