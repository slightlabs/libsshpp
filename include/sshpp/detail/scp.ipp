...
// Generated from src/scp.cpp; see docs/design/09 §9.3.
...
SSHPP_INLINE Reader::Reader(Session& session, const RemotePath& location, bool recursive)
    : native_(ssh_scp_new(session.native_handle(),
                          mode_to_native(recursive ? Mode::read_recursive : Mode::read),
                          location.str().c_str())),
      core_(session.core_) {}

SSHPP_INLINE Reader::~Reader() {
    if (native_ != nullptr) {
...
SSHPP_INLINE Reader::Reader(Reader&& other) noexcept
    : native_(std::exchange(other.native_, nullptr)), core_(std::move(other.core_)) {}

SSHPP_INLINE Reader& Reader::operator=(Reader&& other) noexcept {
    if (this != &other) {
...
SSHPP_INLINE Result<void> Reader::try_init() {
...
SSHPP_INLINE Result<std::optional<Request>> Reader::try_next() {
...
SSHPP_INLINE Result<void> Reader::try_accept() {
...
SSHPP_INLINE Result<void> Reader::try_deny(std::string_view reason) {
...
SSHPP_INLINE Result<std::size_t> Reader::try_read(MutableByteView buf) {
...
SSHPP_INLINE Result<std::uint64_t> Reader::try_read_to(std::ostream& out, ProgressCallback progress) {
...
SSHPP_INLINE Writer::Writer(Session& session, const RemotePath& destination_dir, bool recursive)
    : native_(ssh_scp_new(session.native_handle(),
                          mode_to_native(recursive ? Mode::write_recursive : Mode::write),
                          destination_dir.str().c_str())),
      core_(session.core_) {}

SSHPP_INLINE Writer::~Writer() {
    if (native_ != nullptr) {
        ssh_scp_close(native_);
        ssh_scp_free(native_);
    }
...
SSHPP_INLINE Writer::Writer(Writer&& other) noexcept
    : native_(std::exchange(other.native_, nullptr)), core_(std::move(other.core_)) {}

SSHPP_INLINE Writer& Writer::operator=(Writer&& other) noexcept {
    if (this != &other) {
        if (native_ != nullptr) { ssh_scp_close(native_); ssh_scp_free(native_); }
        native_ = std::exchange(other.native_, nullptr);
        core_ = std::move(other.core_);
...
SSHPP_INLINE Result<void> Writer::try_init() {
...
SSHPP_INLINE Result<void> Writer::try_push_file(std::string_view name, std::uint64_t size, std::filesystem::perms perms) {
...
SSHPP_INLINE Result<void> Writer::try_push_directory(std::string_view name, std::filesystem::perms perms) {
...
SSHPP_INLINE Result<void> Writer::try_leave_directory() {
...
SSHPP_INLINE Result<void> Writer::try_write(ByteView data) {
...
SSHPP_INLINE Result<std::uint64_t> Writer::try_write_from(std::istream& in, std::uint64_t size, ProgressCallback progress) {
...
SSHPP_INLINE Result<std::uint64_t> try_download(Session& session, const RemotePath& remote,
...
SSHPP_INLINE Result<std::uint64_t> try_upload(Session& session, const std::filesystem::path& local,
