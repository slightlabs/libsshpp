...
// Generated from src/known_hosts.cpp; see docs/design/09 §9.3.
...
SSHPP_INLINE KnownHosts::KnownHosts(std::filesystem::path user_file, std::filesystem::path global_file)
    : user_file_(std::move(user_file)), global_file_(std::move(global_file)) {}

SSHPP_INLINE KnownHosts KnownHosts::default_files() {
    return KnownHosts(default_user_known_hosts(), "/etc/ssh/ssh_known_hosts");
}

SSHPP_INLINE Result<void> KnownHosts::add(std::string_view host, std::uint16_t port, const Key& key,
...
SSHPP_INLINE Result<KnownHostsEntry> KnownHosts::parse_line(std::string_view host, std::string_view line) {
