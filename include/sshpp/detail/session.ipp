...
// Generated from src/session.cpp; see docs/design/09 §9.3.
...
SSHPP_INLINE Session::Session(const SessionOptions& opts) : options_(opts) {
...
SSHPP_INLINE Result<void> Session::try_set_options(const SessionOptions& opts) {
    if (!core_) return ErrorInfo{make_error_code(errc::invalid_handle), "", "Session::try_set_options"};
    options_ = opts;
    return apply_options(core_->raw(), opts);
}

SSHPP_INLINE void Session::set_options(const SessionOptions& opts) { try_set_options(opts).throw_if_error(); }

SSHPP_INLINE Result<void> Session::try_connect() {
...
SSHPP_INLINE void Session::connect() { try_connect().throw_if_error(); }

SSHPP_INLINE void Session::disconnect() noexcept {
    if (core_ && core_->valid()) {
        ssh_disconnect(core_->raw());
    }
}

SSHPP_INLINE bool Session::is_connected() const noexcept {
    return core_ && core_->valid() && ssh_is_connected(core_->raw()) != 0;
}

SSHPP_INLINE int Session::socket_fd() const noexcept {
    return core_ && core_->valid() ? static_cast<int>(ssh_get_fd(core_->raw())) : -1;
}

SSHPP_INLINE Result<std::string> Session::try_server_banner() const {
...
SSHPP_INLINE NegotiatedAlgorithms Session::negotiated() const {
...
SSHPP_INLINE Result<PublicKey> Session::try_server_public_key() const {
...
SSHPP_INLINE Result<KnownHostsStatus> Session::try_check_known_host() const {
...
SSHPP_INLINE Result<void> Session::try_update_known_hosts() {
...
SSHPP_INLINE Result<void> Session::try_verify_host_key(const HostKeyVerifier& verifier) {
...
SSHPP_INLINE void Session::verify_host_key(const HostKeyVerifier& verifier) {
    try_verify_host_key(verifier).throw_if_error();
}

SSHPP_INLINE Result<AuthMethods> Session::try_auth_methods() {
...
SSHPP_INLINE Result<AuthStatus> Session::try_authenticate(const Authenticator& authenticator) {
...
SSHPP_INLINE void Session::authenticate(const Authenticator& authenticator) {
...
SSHPP_INLINE Result<AuthStatus> Session::try_authenticate(std::initializer_list<const Authenticator*> chain) {
...
SSHPP_INLINE Result<Channel> Session::try_open_channel() {
...
SSHPP_INLINE Channel Session::open_channel() { return try_open_channel().value(); }

#if SSHPP_WITH_SFTP
SSHPP_INLINE Result<sftp::Sftp> Session::try_open_sftp() {
...
#if SSHPP_WITH_SCP
SSHPP_INLINE Result<scp::Reader> Session::try_open_scp_read(const RemotePath& location, bool recursive) {
...
SSHPP_INLINE Result<scp::Writer> Session::try_open_scp_write(const RemotePath& destination_dir, bool recursive) {
...
SSHPP_INLINE void Session::set_log_level(LogLevel level) {
    if (!core_) return;
    (void)level;
}
