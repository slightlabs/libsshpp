...
// Generated from src/server_message.cpp; see docs/design/09 §9.3.
...
SSHPP_INLINE Message::Message(Message&& other) noexcept
    : native_(std::exchange(other.native_, nullptr)), session_(other.session_),
      replied_(std::exchange(other.replied_, true)) {}

SSHPP_INLINE Message& Message::operator=(Message&& other) noexcept {
    if (this != &other) {
        if (native_ != nullptr) {
            if (!replied_) ssh_message_reply_default(native_);
            ssh_message_free(native_);
...
SSHPP_INLINE MessageType Message::type() const noexcept {
...
SSHPP_INLINE Key Message::auth_public_key() const {
...
SSHPP_INLINE Result<void> Message::try_reply_auth_failure(bool partial) {
...
SSHPP_INLINE Result<void> Message::try_reply_auth_pk_ok() {
...
SSHPP_INLINE Result<void> Message::try_reply_default() {
...
SSHPP_INLINE std::string_view Message::exec_command() const noexcept {
    if (native_ == nullptr) return {};
    const char* c = ssh_message_channel_request_command(native_);
    return c ? std::string_view(c) : std::string_view{};
}

SSHPP_INLINE std::string_view Message::subsystem_name() const noexcept {
    if (native_ == nullptr) return {};
    const char* s = ssh_message_channel_request_subsystem(native_);
    return s ? std::string_view(s) : std::string_view{};
}

SSHPP_INLINE std::pair<std::string_view, std::string_view> Message::env_pair() const noexcept {
    if (native_ == nullptr) return {};
...
SSHPP_INLINE Result<void> Message::try_reply_success() {
...
SSHPP_INLINE Result<void> Message::try_reply_failure() {
